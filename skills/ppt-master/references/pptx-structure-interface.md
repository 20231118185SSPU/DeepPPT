...
> **DeepPPT2 interface mapping.** This document defines the Master / Layout /
> placeholder / native-object part contract. The repo's Generate pipeline
> exports **flat** decks by default (`pptx_structure.mode: flat`): every shape
> lives in the slide body, one project-owned Master plus one Blank Layout is
> materialized from the color/typography lock, and no template Master/Layout
> parts are authored. Since the Phase 6 exporter migration, **structured**
> export is wired and enabled by declaring `pptx_structure.mode: structured`
> in `spec_lock.md` together with `template_reuse_scope: mirror|layout`; the
> exporter compiles the declared Master/Layout/placeholder contract into real
> `p:sldMaster` / `p:sldLayout` parts. This document describes that wired mode,
> not a target contract.
