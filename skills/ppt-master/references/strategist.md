<!-- PHASE: Strategist (Steps 1-5). DO NOT read executor-base.md or modes/*.md at this stage — those are Executor-phase documents. -->
...
> **Default presentation surface — Confirm UI.** Deliver the bundled package through the interactive page as a **three-stage flow**, writing one file per stage to `<project>/confirm_ui/` (full schema: [`scripts/docs/confirm_ui.md`](../scripts/docs/confirm_ui.md)): `recommendations.stage1.json` (communication contract — required BCP-47 `primary_language` + open prose `audience` / `communication_intent` / `audience_outcome` / `core_message` / `delivery_context` / `artifact_afterlife` / `content_divergence`, plus `recommend.canvas`) → after Stage-1 confirmation `recommendations.stage2.json` (the complete deck solution: **≥3 coordinated `design_directions`** each bundling visual style, icons, 6-role color palette, language-aware typography, conditional image strategy; mandatory `custom_candidates` for mode / visual style; optional `template_application`; `page_count`; `recommend` for reading mode / mode / visual_style / icons / image usage) → after Stage-2 confirmation `recommendations.stage3.json` (production mechanics only: `recommend` for conditional `image_ai_path`, `formula_policy`, `generation_mode`, `refine_spec`, plus top-level `proactive_speaker_notes` (true) / `proactive_custom_animations` (false) / `proactive_narration_audio` (false)). Each stage file declares `"stage": "stageN"`. You still author everything — enumerable fields name a recommended `id`; generative fields (color `palette`, CJK + Latin typography, generated-image style) each carry **≥3 distinct candidates**; the deck's **visual style** (§d Layer 2) is carried by the ≥3 safe / shifted / bold `design_directions` bundles (the optional `visual_style_spectrum` chip mechanism remains renderable by the page but is no longer the required contract) — creative recommendations always offer real choice, never a single silent option, same hard rule and thinking as h.5. Honest-shortfall exception (mirrors h.5): if the constraints genuinely cannot yield 3 non-conflicting options, present the smaller set and say why — never pad with duplicates or known-conflicting fillers. **Always also print the recommendations + URL in chat** as the always-valid fallback. On each wait, read `<project>/confirm_ui/result.json` for that stage (`generation_mode: "split"` / `refine_spec: true` / each proactive boolean are explicit user choices). Skip the page if the user wants chat-only, and mirror the same three stages in chat. Full launch flow, field rules, and JSON schema live in [SKILL.md Step 4](../SKILL.md) + [`scripts/docs/confirm_ui.md`](../scripts/docs/confirm_ui.md) — don't restate them here. The page is a confirmation surface only.
...
- No user description → **present a personality spectrum, not one safe pick** (this is the lever against "every deck looks the same" — the visual style is what most determines a deck's character, so it gets real choice, same hard rule and thinking as h.5). Author **≥3 distinct styles** from the index's auto-selection table spanning *safe* (the industry-norm recommendation) → *shifted* (an alternate one tick more expressive) → *bold* (a characterful style that challenges the default — `brutalist` / `zine` / `memphis` / `ink-wash` / `vintage-poster` etc., whenever the content can carry it). Give each a one-line **temperament tag + real-world analogy** (like h.5's "like an Economist feature"). In the Confirm UI, deliver the three as **≥3 coordinated `design_directions` bundles** in Stage 2 (each bundling the style with a 6-role color palette, language-aware typography, icon library, and conditional image strategy — see [`scripts/docs/confirm_ui.md`](../scripts/docs/confirm_ui.md)) **and present the same three in chat** as the always-valid fallback; set `recommend.visual_style` to the *safe* pick as the pre-selected default. (The optional `visual_style_spectrum` chip mechanism remains renderable by the page but is no longer the required contract.) The user may pick any of the three, a style outside them, or Custom. Honest-shortfall exception (mirrors h.5): if the content genuinely supports fewer than 3 non-gimmicky directions, present the smaller set and say why — never pad with a style that fights the content.
...
```json
{
  "providers": ["quicklatex", "codecogs", "mathpad", "wikimedia"],
  "items": [
    {
      "id": "formula_001",
      "latex": "E = mc^2",
...
5. **Generate execution lock**: read `templates/spec_lock_reference.md` and produce `projects/<project_name>.../spec_lock.md` — a distilled, machine-readable short form of the color / typography / icon / image / **page_rhythm** / **page_layouts** / **page_charts** / **decisions** decisions above. This file is what the Executor re-reads before every page (see [executor-base.md](executor-base.md) §2.1). The values in `spec_lock.md` MUST exactly match the decisions recorded in `design_spec.md`; if they ever diverge, `spec_lock.md` wins and `design_spec.md` should be treated as historical narrative. Do not add a `page_expression` section; that content belongs only in the sibling JSON.
   ...
   - **Beautify-mode page_rhythm derivation**: when the project is a beautify type (`analysis/beautify_layout_analysis.json` exists), `page_rhythm` MUST be derived from the layout analysis — do not use generic assignment or default all content pages to `dense`. Derivation rules: `cover` / `ending` / `chapter` → `anchor`; `page_mode=emotional` (quote, transition, hero moment) → `breathing`; `page_mode=rational` + layout_family in {timeline, process_flow, hierarchy, evidence_chain} → `dense`; `layout_family` = quote/insight → `breathing`. The analysis-derived rhythm supersedes any generic heuristic — if the analysis says a page is `breathing`, it stays `breathing` even if the Strategist would normally default it to `dense`.
