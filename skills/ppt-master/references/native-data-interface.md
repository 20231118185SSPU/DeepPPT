...
> **DeepPPT2 interface mapping.** Native data objects (charts, tables, pattern
> fills, `data-pptx-replace-with`) are the vector/data-object path activated by
> the `--native-charts-and-tables` export flag. The default flat pipeline
> renders charts as grouped SVG shapes, and raster assets stay under
> `spec_lock.md` `## images` — the Executor's image-prompt/asset list contract.
> This document does not replace that image list; it defines how data visuals
> are authored natively when the native-objects route is enabled.
