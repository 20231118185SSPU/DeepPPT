...
> **DeepPPT2 interface mapping.** These markers are the upstream compiler-hint
> vocabulary the Executor may emit in SVG. In this repo they sit *below* the two
> machine contracts Executor reads per page — `spec_lock.md` (visual tokens) and
> `page_expression.json` (per-page question/assertion/evidence/visual_act/takeaway)
> — so marker values must never contradict those locks. The Master/Layout/placeholder
> markers (`data-pptx-master`, `data-pptx-layout`, `data-pptx-layer`,
> `data-pptx-placeholder`) compile only under the explicit
> `pptx_structure.mode: structured` declaration in `spec_lock.md`; the default
> `flat` export keeps every object Slide-local and ignores them
> (see [`pptx-structure-interface.md`](./pptx-structure-interface.md)).
