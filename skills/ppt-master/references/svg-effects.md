...
> **DeepPPT2 interface mapping.** Effects authored from this catalog must map to
> OOXML `a:effectLst` semantics when native/structured authoring is enabled. In the
> current flat pipeline, shadow / overlay / stroke / text effects are governed by
> this repo's `shared-standards.md` §§6–7 and baked in by `finalize_svg.py`; this
> document extends that surface (glow, gradient, geometry operations) for preset and
> structured authoring. The geometry engine here also backs
> `prstgeom_to_svg.py`'s reverse conversion for the beautify route.
...
[`drawingml/paths.py`](../scripts/svg_to_pptx/drawingml/paths.py) (the migrated
package module; the pre-refactor flat `drawingml_paths.py` has been retired);
