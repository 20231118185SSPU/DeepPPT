...
**Page-type text policy (HARD rule)**:

| Page type | `text_policy` | Prompt suffix | Reason |
|---|---|---|---|
| Cover (`01_cover`) | `none` | Append: "no text, no letters, no words, no typography in the image" | Text is SVG overlay; in-image text conflicts |
| TOC (`02_toc`) | `none` | Append: "no text, no letters, no words, no typography in the image" | Same |
| Chapter/transition (`02_chapter`) | `none` | Append: "no text, no letters, no words, no typography in the image" | Same |
| Ending (`04_ending`) | `none` | Append: "no text, no letters, no words, no typography in the image" | Same |
| Big quote | `none` | Append: "no text, no letters, no words, no typography in the image" | Quote text is SVG rendered |
| **Content/explanation (`03_content`)** | **`embedded` (allowed)** | May include: infographic labels, data annotations, diagram text | Image itself carries explanatory content; SVG adds title + bullets |

> For non-content pages, the "no text" suffix is **mandatory** — do not omit it even when the prompt already seems visual-only. For content pages with `content_mode: image_with_text`, the prompt SHOULD describe the explanation content the image should contain (e.g., "a flowchart showing user login → verification → authorization → dashboard").
...
**Size matching rule (HARD rule)**:

AI-generated images MUST match the target SVG layout slot dimensions exactly. The slot size comes from the template's JSON `image_slots` array (e.g., `{"width": 550, "height": 420}`).

| Scenario | Action |
|---|---|
| Slot size known from template JSON | Set `target_width` and `target_height` in `image_prompts.json` to match slot dimensions exactly |
| Full-page hero image | Generate at 1280×720 |
| Slot size not specified | Default to 1280×720; Executor uses `preserveAspectRatio="xMidYMid slice"` to crop |

**Why**: generating at 1280×720 for a 550×420 slot causes the image to be cropped awkwardly — important content at the edges is lost. Generating at the exact slot size ensures the full image content is visible.

**Size mapping table** (typical content page slot sizes):

| Template | Slot | Target size (w×h) |
|---|---|---|
| `image_hero` | hero | 1280×720 |
| `image_text` | left image | 550×420 |
| `text_with_sidebar` | sidebar image | 350×300 |
| `hero band` / wide image | top/bottom band | 1160×425 |
| Card-internal small image | card slot | 180×120 |
...
0. **Detect web-to-AI fallback** — if the row's original `Acquire Via` was `web` but the image is being generated by AI (web search failed, row was auto-converted), switch to **informational mode** (see §3.1). This is the critical distinction: the prompt must describe *what the image shows* (data, process, real-world content), not *what it feels like* (atmosphere, mood). A deep-dive page with an atmospheric AI concept image instead of informational content is a quality failure.
1. **Resolve the page-image job** — read the page's `core_argument`, `content_bullets`, `content_density`, `visual_need`, and planned layout. Decide whether the image is (a) omitted, (b) an informational explanation, (c) an atmosphere/concept support, or (d) a hero voice. If a page has too much text, prefer an AI explanatory diagram / infographic that carries structure, not another decorative picture. If a page has little text, prefer atmosphere, concept, or auxiliary imagery that leaves room for SVG wording.
...
5. **Resolve reference-image policy** — run §3.2 before using any `reference_image`. High-ambiguity subjects default to text-to-image unless source, semantic match, confidence, and fallback are explicitly recorded.
6. `read_file references/image-type-templates/<type>.md` (only if not already read — types are commonly reused across images in one deck)
7. **Assemble the prompt** by combining:
   ...
   - The page information structure: `core_argument`, summarized `content_bullets`, density, and whether SVG text overlays or sits beside the image
...
### 3.1 Informational mode — web-to-AI fallback

When a row was originally `Acquire Via: web` but falls back to AI generation (web search failed, no usable stock found), the prompt must follow **informational mode** rules. This is the operational bridge between the Type A/B distinction defined in the Strategist and Executor layers:

| | Type A (概念图) — default AI mode | Type B (讲解配图) — informational mode |
|---|---|---|
| **Question** | "What does it feel like?" | "What does it show?" |
| **Content** | Atmosphere, mood, color fields, abstract shapes | Specific data, processes, real-world objects, comparisons |
| **Composition** | §4.1 primitives (A–E), especially D (atmospheric) | 11 information types (infographic / flowchart / framework / matrix / cycle / funnel / pyramid / comparison / timeline / map / scene) |
| **Used for** | Cover, transition, ending backgrounds | Deep-dive, comparison, data, timeline pages |

**Informational mode prompt rules** (append to §4 assembly when fallback is detected):

1. **Lead with content, not atmosphere** — the prompt's opening sentences describe *what the image shows* (concrete nouns: a specific chart shape, a named process, a recognizable object), not *what mood it creates*.
2. **Choose an information type** — pick from the 11 type templates based on the row's `Purpose`. Even if the page is `hero_page`, informational mode uses a type template instead of the atmospheric primitive (Primitive D).
3. **Name the domain** — if the deck's subject is technical (engineering, science, finance), spell out the field's visual conventions per §4.2.
4. **Avoid "concept art" vocabulary** — do not use phrases like "abstract", "atmospheric", "mood", "ethereal", "dreamlike", "conceptual". These trigger Type A output.
5. **Include reference content** — translate the row's `Reference` field into specific visual elements. If the reference says "show the 5-step PM process", the prompt should describe 5 distinct step nodes with connecting arrows, not a vague "process visualization".

**Example — wrong (Type A leak into deep-dive page)**:

> ❌ Atmospheric gradient background in deep blue and gold, abstract flowing shapes suggesting innovation and progress, ethereal digital landscape...

**Example — correct (informational mode for the same page)**:

> ✅ A clean 5-step horizontal process diagram. Five rounded rectangular nodes in primary deep navy `#1E3A5F` are connected by directional arrows in accent gold `#D4AF37`. Each node contains a simplified icon representing: research (magnifying glass), design (pencil), build (gear), test (checkmark), launch (rocket). Icons are rendered as simple line art in white. Background is secondary light gray `#F8F9FA`. The layout flows left to right with even spacing. Composed as a 1200×600px local infographic block.

### 3.2 Reference-image gating for img2img

`reference_image` is an optional precision tool, not permission to let a random search result steer the image. A wrong reference image is worse than no reference image.

| Subject risk | Default | Img2img allowed only when |
|---|---|---|
| Named person / character / public figure / founder / speaker | Text-to-image | Reference source is official/user-provided or otherwise clearly authoritative; semantic match is exact; confidence ≥0.85; fallback plan recorded |
| Real place / landmark / event | Text-to-image | Source identifies the same place/event; date/context matches the page claim; confidence ≥0.85 |
| Product / object / brand asset | Text-to-image | User supplied it, or source URL/provenance proves the same object; confidence ≥0.75 |
| Abstract concept / atmosphere / texture | No reference needed | Use text-to-image; do not attach decorative references |

**Hard rule**: Do not use a web-search reference image for high-ambiguity subjects merely because the filename or search result contains the same name. Person-name collisions, fictional-character variants, similarly named places, and historical-event aliases all route to text-to-image unless the manifest proves the match.

**Manifest requirement when `reference_image` is present**:
...
"reference_image_policy": {
  ...
  "confidence": 0.92,
  "subject_risk": "person",
  "fallback": "If reference confidence fails, generate a non-likeness symbolic scene from the page prompt."
}
```

**Runtime behavior**: `image_gen.py --manifest` validates `reference_image_policy`. If source/provenance, semantic match, confidence, or approval is missing, it skips img2img for that row, records `reference_image_review`, and generates from the text prompt instead.
...
> **Informational mode override**: when §3 Step 0 detected a web-to-AI fallback, replace the §4.1 atmospheric primitive (Primitive D) with an information type from the 11 type templates. Lead with content nouns, not mood adjectives. See §3.1 for the full rules and examples.
...
[Page content structure — what the page argues, the 2-4 content points the image should support, content density, and whether SVG text overlays the image or sits beside it].
  ...
  "items": [
    {
      ...
      "aspect_ratio": "16:9",
      "image_size": "2K",
      "image_intent": "Create the cover's emotional hook while SVG owns the title.",
      "page_evidence": "P01 core_argument: ...; content density: low; cover impact: ...",
      "text_image_relationship": "Full-bleed image with editable SVG title overlay on a calm area.",
      "fallback_plan": "If the reference is not verified, generate an atmospheric non-likeness backdrop.",
    ...
    },
    {
      ...
      "aspect_ratio": "4:3",
      "image_size": "1K",
      "image_intent": "Turn the page's three-part method into a visual framework.",
      "page_evidence": "P05 core_argument + three content bullets from detailed_outline.json.",
      "text_image_relationship": "Image explains structure; SVG title and takeaway remain editable.",
      "fallback_plan": "If generation fails, use native SVG framework boxes with icons.",
...
| `items[].image_intent` | yes | Page strategy | The communication job of the image on this page: emotional hook, explanation, evidence, transition, or support. |
| `items[].page_evidence` | yes | `detailed_outline.json` / `design_spec.md §IX` | The page claim, content bullets, density, or layout reason that caused this image to exist. Must be specific enough to audit why the prompt says what it says. |
| `items[].text_image_relationship` | yes | Layout plan | How the image and editable SVG text share work: overlay, side-by-side, image-owned labels, SVG-owned labels, or no text interaction. |
| `items[].fallback_plan` | yes | Image strategy | What to do if AI generation fails or a reference is rejected: text-to-image, native SVG diagram, placeholder, web search, or manual asset. |
| `items[].image_size` | no | Container sizing | `512px` / `1K` / `2K` / `4K` |
| `items[].alt_text` | no | Accessibility | Short caption |
| `items[].reference_image` | optional | §3.2 review | **File path or URL** to a vetted reference image for img2img mode. Must be a real file on disk (e.g. `projects/<name>/images/ref/ref_p04_product.png`) or an HTTP(S) URL — **not** a text description. Omit when the subject is high-ambiguity and no authoritative match exists. |
| `items[].reference_image_policy` | conditional | §3.2 review | Required when `reference_image` is present. Records `approved`, `source_url`/provenance, `semantic_match`, `confidence`, `subject_risk`, and `fallback`. Without it, `image_gen.py` skips img2img. |
| `items[].reference_image_review` | no | CLI manages | Runtime audit written by `image_gen.py`: `img2img` or `text-to-image`, confidence threshold, and skip reasons. |
...
| Deep-dive page has atmospheric/mood image instead of informational content | Type A leak — web row fell back to AI without switching to informational mode | Switch to §3.1 informational mode: lead with content nouns, use an information type template, remove atmospheric vocabulary |
...
- **Type A leak**: using atmospheric / mood-based prompts (Primitive D, "abstract", "ethereal") for pages that were originally `Acquire Via: web` and fell back to AI — these must use informational mode per §3.1
- **Concrete subject without reference**: generating any image depicting a named character, person, product, object, real place, or IP-specific subject without `reference_image` — concrete-subject images MUST use img2img when the backend supports it, or at minimum include a vetted reference path for Executor review
- **Batch generation of concrete-subject images without review**: concrete-subject images must have tailored prompts and per-image reference images. Batch mode is acceptable only when every row has its own `reference_image` and the generated files are visually reviewed before SVG generation
- **Mismatched dimensions**: generating images at generic sizes (1K/2K) without first determining the exact SVG layout slot dimensions — always design the layout first, then generate to fit
