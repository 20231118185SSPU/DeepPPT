...
| `items[].reference_image` | conditional | §9 / character-object workflow | **File path or URL** to a reference image for img2img mode. Must be a real file on disk (e.g. `projects/<name>/images/ref/ref_p04_product.png`) or an HTTP(S) URL — **not** a text description. The backend uploads this to `/v1/images/edits` for image-to-image generation when supported. Required for named characters, people, products, objects, places, IP-specific subjects, and any image whose layout depends on a concrete visible subject. Omit only for abstract concept backgrounds / textures. When the value is not a valid file path or URL, `image_gen.py` logs a warning and falls back to text-to-image automatically; the row must then be reviewed before use. |
