...
| `items[].reference_image` | no | §9 / character workflow | **File path or URL** to a reference image for img2img mode. Must be a real file on disk (e.g. `projects/<name>/ref/character.png`) or an HTTP(S) URL — **not** a text description. The backend uploads this to `/v1/images/edits` for image-to-image generation. Required for character depictions per §9 rules; omit for normal text-to-image items. When the value is not a valid file path or URL, `image_gen.py` logs a warning and falls back to text-to-image automatically. |
...
- **Character without reference**: generating any image depicting a named character without `--reference-image` — character images MUST use img2img with a reference portrait from `projects/<name>/ref/`
- **Batch generation of character images**: character images MUST be generated one at a time with tailored prompts and per-image reference images; `--manifest` batch mode is for concept backgrounds only
