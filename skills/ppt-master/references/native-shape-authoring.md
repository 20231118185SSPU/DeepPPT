...
> **DeepPPT2 interface mapping.** The authored-preset contract here matches this
> repo's `scripts/preset_shape_svg.py` and `scripts/pptx_to_svg/preset_authoring.py`,
> which share the `pptx_shapes` catalog (187 locked presets). Rendering-neutral
> metadata these tools emit (source preset name, explicit `avLst` guide formulas)
> must survive `finalize_svg.py` untouched so the reverse converter
> (`prstgeom_to_svg.py`, used by the beautify route) can distinguish a native
> PowerPoint shape from an arbitrary path. Authoring-validation failures surface at
> `svg_quality_checker.py` time; preset fragments are emitted at Executor authoring
> time, not assembled at export.
