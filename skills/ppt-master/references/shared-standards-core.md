...
> **DeepPPT2 interface mapping.** This is the upstream base foundation imported for
> the native-shape / structured-template work. This repo's own
> `references/shared-standards.md` remains the active authority for the current
> Generate pipeline (banned-feature blacklist, canvas formats, post-processing
> steps). Where both apply, core rules are the baseline and the repo's
> `shared-standards.md` plus `docs/rules/` layer repo-specific constraints on top.
