...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        from svg_to_pptx.shape_boolean import (  # noqa: F401
            render_boolean_svg_fragments,
        )
    except ModuleNotFoundError:
        # Requires skia-pathops (optional dependency); the core module itself
        # reports a clear functional error when the library is absent.
        print(
            "Error: the shape-boolean core (svg_to_pptx.shape_boolean) could "
            "not be imported. It requires skia-pathops — install the project "
            "requirements, or run: pip install skia-pathops",
            file=sys.stderr,
        )
        return 1
