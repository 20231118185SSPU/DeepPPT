...
class _LazyPPTXEntries(dict):
    """Dict subclass that reads ZIP entries on demand instead of upfront.

    Reads from the source ZIP lazily and caches in memory. Modified entries
    (via __setitem__) are tracked separately so the output ZIP can stream
    unmodified parts directly from the source.
    """
    ...
    def __init__(self, zip_path: Path) -> None:
        ...
        self._load_source_names()

    def _load_source_names(self) -> None:
        with zipfile.ZipFile(self._zip_path) as zf:
            self._source_names = {i.filename for i in zf.infolist() if not i.is_dir()}

    def _ensure_zip(self) -> zipfile.ZipFile:
        if self._zf is None or self._zf.fp is None:
    ...
    def __contains__(self, key: object) -> bool:
        if dict.__contains__(self, key):
            return True
        return key in self._source_names
...
def apply_plan(
    ...
    *,
    ...
    entries = _LazyPPTXEntries(pptx_path)
    try:
        ...
        with zipfile.ZipFile(output_path, "w", compression=zipfile.ZIP_DEFLATED) as out:
            for name in entries._source_names:
                out.writestr(name, entries[name])
            for name in dict.keys(entries):
                if name not in entries._source_names:
                    out.writestr(name, dict.__getitem__(entries, name))
    finally:
        entries.close()
