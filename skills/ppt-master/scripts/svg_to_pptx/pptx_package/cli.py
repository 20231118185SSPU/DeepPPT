...
''',
    ...
    # DeepPPT2 compatibility surface (kept from the pre-refactor CLI).
    ...
    version_group.add_argument(
        ...
        default=None,
        help='Only generate one version: native (editable shapes, default) or '
             'legacy (PNG-rendered image deck)',
    )
    version_group.add_argument(
        '--native',
        action='store_true',
        default=False,
        help='Generate the native editable-shape version (default behavior)',
    )
    parser.add_argument(
        '--svg-snapshot',
        action='store_true',
        default=False,
        help='Additionally emit a legacy PNG-rendered deck as a sibling '
             'exports/<name>_<timestamp>_svg.pptx',
    )
    parser.add_argument(
        '--no-compat',
        action='store_true',
        default=False,
        help='Disable Office compatibility mode (PNG fallback embedding) for '
             'legacy/image-mode exports',
    )
    parser.add_argument(
        '--cache-dir',
        type=str,
        ...
        help='Cache directory for SVG->PNG renders (default: '
             '<project>/.cache/svg_png). Cache key uses SVG content hash + '
             'size + renderer; safe across renderer switches. Removed '
    ...
    parser.add_argument(
        ...
        help='Keep the SVG->PNG cache directory after export '
             '(default: removed on success to keep the project clean).',
    )
    parser.add_argument(
        '--workers',
        type=int,
    ...
    if args.quick_generate:
        ...
        if args.svg_snapshot:
            conflicts.append('--svg-snapshot (Quick publishes one native PPTX)')
        if args.only == 'legacy':
            conflicts.append('--only legacy (Quick publishes one native PPTX)')
        if conflicts:
            print(
    ...
    # DeepPPT2 version-selection surface: default native-only; --only legacy
    # forces the PNG-rendered image deck; --svg-snapshot adds it as a sibling.
    only_mode = args.only
    if only_mode == 'native' or args.native:
    ...
    # Legacy/image-mode exports are PNG-rendered (use_native_shapes=False);
    # --no-compat disables the PNG fallback embedding for that mode.
    ...
    if args.output:
        ...
        legacy_path = (
            native_path.parent / f"{native_path.stem}_svg{native_path.suffix}"
            if gen_legacy
            else None
        )
    else:
        ...
        if gen_legacy:
            legacy_path = exports_dir / f"{project_name}_{timestamp}{narrated_tag}_svg.pptx"
        else:
            legacy_path = None
    ...
    shared_kwargs = dict(
        ...
        cache_dir=(
            None
            if args.no_cache
            else Path(args.cache_dir).expanduser().resolve()
            if args.cache_dir
            else project_path / ".cache" / "svg_png"
        ),
    )
    ...
    try:
        ...
        if success and gen_legacy and legacy_path is not None:
            # Legacy PNG-rendered deck (DeepPPT2 --only legacy / --svg-snapshot):
            # the same SVG roster rendered to embedded images instead of native
            # DrawingML shapes. The conversion trace is native-only.
            if verbose:
                print(f"  Legacy image deck: {legacy_path}")
            success = create_pptx_with_native_svg(
    ...
    # DeepPPT2 cache lifecycle: drop the PNG cache on success unless the
    # caller asked to keep it (--keep-cache); failures keep it for retries.
    if success and not args.keep_cache and not args.no_cache:
        cache_root = (
        ...
        if cache_root.is_dir():
            try:
                ...
                if verbose:
                    print(f"  [warn] PNG cache cleanup skipped: {exc}")
