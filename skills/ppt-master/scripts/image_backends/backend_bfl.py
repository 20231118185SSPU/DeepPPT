...
Purpose:
    Submit PPT Master image prompts to the Black Forest Labs FLUX API and save
    the returned image through shared backend helpers.

Used by:
    image_gen.py when the selected image backend is "bfl".

Configuration:
    BFL_API_KEY   Required API key.
    BFL_BASE_URL  Optional API base URL override.
    BFL_MODEL     Optional model override.
...
def generate(prompt: str,
    ...
    if reference_image is not None:
        raise ValueError(
            "Backend 'bfl' does not support img2img (reference_image). "
