...
Purpose:
    Generate PPT Master image assets via MiniMax image generation endpoints.

Used by:
    image_gen.py when the selected image backend is "minimax".

Configuration:
    MINIMAX_API_KEY   Required API key.
    MINIMAX_BASE_URL  Optional API base URL override.
...
def generate(prompt: str,
    ...
    if reference_image is not None:
        raise ValueError(
            "Backend 'minimax' does not support img2img (reference_image). "
