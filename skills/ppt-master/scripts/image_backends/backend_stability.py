...
Purpose:
    Generate PPT Master image assets through Stability AI image generation
    endpoints.

Used by:
    image_gen.py when the selected image backend is "stability".

Configuration:
    STABILITY_API_KEY   Required API key.
    STABILITY_BASE_URL  Optional API base URL override.
    STABILITY_MODEL     Optional model override.
...
def generate(prompt: str,
    ...
    if reference_image is not None:
        raise ValueError(
            "Backend 'stability' does not support img2img (reference_image). "
