...
Purpose:
    Generate PPT Master image assets through Volcengine/Ark Seedream image
    generation endpoints.

Used by:
    image_gen.py when the selected image backend is "volcengine".

Configuration:
    VOLCENGINE_API_KEY / ARK_API_KEY  Required API key.
    VOLCENGINE_BASE_URL               Optional API base URL override.
    VOLCENGINE_MODEL                  Optional model override.
...
def generate(prompt: str,
    ...
    if reference_image is not None:
        raise ValueError(
            "Backend 'volcengine' does not support img2img (reference_image). "
