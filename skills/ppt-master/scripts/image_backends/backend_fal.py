...
Purpose:
    Submit PPT Master image prompts to fal.ai hosted image models and download
    the generated image asset.

Used by:
    image_gen.py when the selected image backend is "fal".

Configuration:
    FAL_KEY / FAL_API_KEY  Required API key.
    FAL_BASE_URL           Optional API base URL override.
    FAL_MODEL              Optional model override.
...
def generate(prompt: str,
    ...
    if reference_image is not None:
        raise ValueError(
            "Backend 'fal' does not support img2img (reference_image). "
