...
Purpose:
    Generate PPT Master image assets through Zhipu/BigModel GLM image
    generation endpoints.

Used by:
    image_gen.py when the selected image backend is "zhipu".

Configuration:
    ZHIPU_API_KEY / BIGMODEL_API_KEY  Required API key.
    ZHIPU_BASE_URL                    Optional API base URL override.
    ZHIPU_MODEL                       Optional model override.
...
def generate(prompt: str,
    ...
    if reference_image is not None:
        raise ValueError(
            "Backend 'zhipu' does not support img2img (reference_image). "
