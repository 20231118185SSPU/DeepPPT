...
DeepPPT extensions on top of the upstream base: bundled template previews
(``/template-file/<kind>/<template_id>/...``), read-only ``template_route`` /
``layout_preview`` injection into ``/api/recommendations``, and the full
three-kind ``ai-image-comparison`` reference surface (rendering / palette /
type).
...
_ICON_LIBRARY_DIR = _SKILL_DIR / 'templates' / 'icons'
_AI_IMAGE_COMPARISON_DIR = _SKILL_DIR / 'references' / 'ai-image-comparison'
# DeepPPT extension: bundled template library roots for /template-file previews.
_TEMPLATE_KIND_DIRS = {
...
def _build_ai_image_comparison() -> dict:
    # DeepPPT extension: keep all three comparison kinds (rendering / palette /
    # type) served; the base upstream surface only exposes rendering presets.
    return {
...
def _template_route(project_path: Path) -> dict:
    """DeepPPT extension: read-only template route/discovery data for the page."""
...
def _layout_preview(project_path: Path, limit: int = 12) -> dict:
    """DeepPPT extension: pre-generation page plan preview from the detailed outline."""
...
def create_app(
    ...
    def template_file(kind: str, template_id: str, filename: str):
        """DeepPPT extension: serve bundled template preview SVGs read-only."""
        base = _TEMPLATE_KIND_DIRS.get(kind)
        if base is None:
    ...
    def get_recommendations():
        ...
        # DeepPPT extensions: inject read-only template route and detailed-outline
        # layout preview data — never authored by Strategist, presentation-only.
