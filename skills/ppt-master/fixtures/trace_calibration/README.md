...
Inputs: `trace.jsonl` (10 events) + `quality/harness.json` + `quality/pptx_quality.json`.

Command: `python3 skills/ppt-master/scripts/run_summary.py <copy> -o <tmp>/run_summary.json`

Expected summary (contract):
- `route: "generate"`, `slide_count: 3` (from pptx_quality.json), `events_total: 10`.
...
- `image_attempts: 2` (image_gen_attempt + image_search; the `images` stage name
  must not count).
- `annotations.*: null` + not-wired warnings (no writer emits those events).
