...
## Shared stage — two route-specific integration paths

This stage is the **single shared audio-generation surface** for both routes that
produce narrated decks. Steps 1–4 (language → voice catalog → one-shot
confirmation → per-slide audio generation via `notes_to_audio.py`) are route
agnostic and produce the same artifact contract: `notes/<stem>.md` →
`audio/<stem>.mp3` (edge also writes `notes/subtitles/<stem>.srt`). The final
embedding mechanism is **route-specific by design** — never run both on the same
deck:

| Route | Embedding path | Entry |
|---|---|---|
| Generate PPTX (generated projects) | Export-time: `svg_to_pptx.py <project> --recorded-narration audio` (per-slide media + auto-advance timings during conversion) | Steps 4–5 below |
| Enhance Native PPTX (arbitrary finished PPTX) | Post-export OOXML patch: `native_enhance_pptx.py apply <project>` (Step 9 of [`../native-enhance-pptx.md`](../native-enhance-pptx.md)) | Stop after Step 4; do **not** run the export-time integration |

**Hard rule — no double embedding**: a deck's narration is embedded exactly
once, through the owning route's path. The Enhance route integrates audio
through `native_enhance_pptx.py apply` instead of `--recorded-narration`.
...
## Step 4.5: Video export (optional branch)

When the user asks for a video ("导出视频" / "export as video" / "make an mp4"), run the automatic PowerPoint encoder — never claim availability without probing:

```bash
# 1. Probe PowerPoint COM availability (Office 2016+ on Windows only)
python3 skills/ppt-master/scripts/powerpoint_video.py --check
```

If `--check` fails (non-Windows, no PowerPoint, or the COM probe times out), report that automatic export is unavailable and offer the manual PowerPoint export path (`docs/audio-narration.md` "Export as video"). If it passes:

```bash
# 2. Encode the narrated PPTX (default 1080p / 30fps / quality 85)
python3 skills/ppt-master/scripts/powerpoint_video.py \
  "<project>/exports/<deck>.pptx" -o "<project>/exports/<deck>.mp4"
```

PowerPoint's native encoder plays the recorded narration + auto-advance timings and keeps the deck's own transitions/animations; it does not re-render the slides. On failure the narrated PPTX remains the deliverable and is reported separately.

Optional synced subtitles (requires `pip install stable-ts`; `ffmpeg`/`ffprobe` must be on PATH):

```bash
# 3. Force-align the page SRT transcripts against the encoded video
python3 skills/ppt-master/scripts/video_subtitles.py <project_path> \
  --video "<project>/exports/<deck>.mp4" --language <zh|en|ja|ko> --force
```

Subtitles stay external (`.srt` beside the video); they are never burned in. The `narration_sync.py subtitles --video` command is a diagnostics-only timeline calibration (`notes/subtitles/total.srt`), not a video deliverable.
