---
description: 可选分批生成模式。Executor 每生成 N 页后暂停，等待用户视觉确认，后续批次吸收反馈。仅在用户显式选择后启用；20+ 页长 deck 或视觉风格要求高只能作为提示用户选择的信号。
---

# 批次审阅工作流（Batch Review）

> 用户显式选择后启用。Executor 每生成一批后暂停，收集用户视觉反馈，后续批次吸收修正方向。默认不启用；20+ 页长 deck 或高质量要求项目只能提示用户可选择该模式，不能自动触发。
>
