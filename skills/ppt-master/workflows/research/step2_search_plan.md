...
**输入**: `_research/step1_outline/outline.json` + 项目根目录 `ppt_brief.json`（如存在）
...
读取 `_research/step1_outline/outline.json`，获取所有页面信息。若项目根目录存在 `ppt_brief.json`，必须同时读取它。

**Hard rule**: Topic-only 项目的搜索计划必须继承 `ppt_brief.json` 的目标、受众、叙事框架、内容边界、资料深度、时效要求、素材策略、来源策略、版权风险和验收标准。不得只根据页面标题生成通用搜索计划。
...
**Mandatory**: `source_strategy` 优先从 `ppt_brief.json.source_strategy` 继承；`asset_targets` 必须体现 `ppt_brief.json.material_strategy` 和 `copyright_and_risk`。当 Brief 标记需要官方资产或人工复核时，维度级计划必须写明对应来源和复核要求。
...
1. **解读 Brief 约束** — 读取目标、受众、叙事功能、内容边界、素材策略和验收标准
2. **解读 data_hint** — 该页需要什么类型的数据
3. **生成搜索关键词** — 3-5 个精准关键词
4. **确定内容类型** — 用于分配 AI 工具
5. **绑定维度** — 写入 `dimension_ids`
6. **继承搜索轮次** — 从维度计划中挑选该页必须执行的 2-4 轮搜索
7. **规划素材** — 写清参考图 / 网络素材 / 截图 / 信息图需求和目标尺寸
8. **评估优先级** — 核心论点页 > 数据支撑页 > 背景页
      ...
      "asset_requirements": [
        {
          "type": "web_asset",
          "purpose": "市场增长图或报告截图",
          "brief_material_strategy": "official report screenshot preferred; generic stock disabled",
...
- [ ] 已继承 `ppt_brief.json` 的目标、受众、叙事框架、素材策略和验收标准
- [ ] `brief_source` 和 `brief_constraints` 已写入 `search_plan.json`
