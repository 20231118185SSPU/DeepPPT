...
| `page_type` | enum | `cover` \| `toc` \| `chapter` \| `content` \| `content_alt` \| `ending` (superset used by template indexing; detailed_outline.json typically uses `cover`/`chapter`/`content`/`ending`) |
| `layout_suggestion` | string | Which layout template variant to use (from `content_pages/` library); empty string for cover/ending. Available values by scene: |
...
| `core_argument` | string | ≤25 chars. The ONE thing this page must communicate |
| `content_bullets` | string[] | 3–5 items, each ≤30 chars. Key points on this page |
...
| `content_bullets` count per page | 3–5 | Merge thin bullets or split dense ones |
| `core_argument` length | ≤25 chars | Compress; use abbreviations if needed |
