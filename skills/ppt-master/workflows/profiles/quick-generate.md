...
[`image-base.md`](../../references/image-base.md) (this repo's image-authority
document, in the position the upstream `executor-image.md` holds),
...
[`image-searcher.md`](../../references/image-searcher.md) for a sourced
web image (query/status data and required on-slide attribution). Load
[`canvas-formats.md`](../../references/canvas-formats.md) only
...
Fix every blocking error and rerun the same command. The final report carries
an SVG source fingerprint; the exporter verifies it matches the current
`svg_output/` before creating the PPTX. Then export:
...
selected by the agent (re-enable notes with `--with-notes`). Do not run
`finalize_svg.py`.

The exporter keeps ordinary backup and postflight behavior. An explicit
`-o <path>.pptx` keeps the ordinary no-backup behavior. On failure, repair the
owning SVG, resource, or optional capability input, rerun the final checker,
then export again; do not create a Design Spec or lock.
