...
整个流程就三步。先装好环境——只需要 Python,见 [快速开始](../../README.md#快速开始)。

1. **把源材料放进** `projects/` —— PDF、DOCX、Markdown、一个网址,或直接要粘贴的文字。
2. **在对话里告诉 AI** 要把什么做成 deck(如果上面准备了模板,把它的路径一起给;否则就是自由设计)。如果只有主题,AI 会先写 PPT Brief 并等待你确认后才开始调研:
   ```
   你：用 projects/q3-report/sources/report.pdf 做一份 PPT
   你：把这份内容做成 PPT：<粘贴你的文字>
   你：做一份关于量子计算商业化前景的 PPT
   ```
3. **拿回可编辑的 `.pptx`**,位于 `exports/<名称>_<时间戳>.pptx` —— 真正的 DrawingML 形状、文本框、图表,在 PowerPoint / Keynote / WPS / LibreOffice 里点开就能改。

生成前 AI 会先走八项确认(默认 Confirm UI,必要时聊天 fallback),锁定模板、格式、页数、视觉方向、图片策略等关键选择;之后内容分析、排版、配图、SVG 生成、质量门禁和导出都由它完成——这就是其它能力围绕的核心环节。
...
完整说明 → [实时预览工作流](../../skills/ppt-master/workflows/stages/live-preview.md)

---

## 转场与动画

导出的 deck 自带真正的 OOXML **页间转场**——不是嵌入视频。页间转场默认开启;页内元素入场动画默认关闭,每页会整体出现。只有显式使用 `-a auto`、其它动画效果或 `animations.json` 时,元素才会逐个入场。想要特定顺序、效果或时序时再定制即可。
