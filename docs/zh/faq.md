...
可以。直接告诉 AI 你想做的主题或场景（如"做一个关于宫崎骏的 PPT"、"介绍我们公司新产品"）。没有源材料时，AI 会先运行 **ppt-briefing**，生成 `ppt_brief.md` / `ppt_brief.json` 并等待你明确确认；只有确认 Brief 后，才进入 **deep-research** 的 7 步调研（大纲生成 → 搜索需求拆分 → 逐页搜索 → 汇总 → 结构化分析 → 叙事构建 → 视觉策略），产出结构化调研产物后进入主流程生成幻灯片。
...
只有信号偏重的场景（页数 ≥ 18 / 源材料很厚 / 走过深度调研累积大量搜索结果），AI 才会在策略师阶段给出**两段式（拆分模式）**的可选提示：第一阶段（八项确认 + 图片获取）结束后停止当前对话；你新开聊天窗口，输入 `继续生成 projects/<项目名>` 进入第二阶段（SVG 生成 + 导出）。新会话从磁盘重新加载 `design_spec` / `spec_lock` / `sources` / `images` 继续执行。
