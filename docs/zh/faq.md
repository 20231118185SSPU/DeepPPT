...
可以。直接告诉 AI 你想做的主题或场景（如"做一个关于宫崎骏的 PPT"、"介绍我们公司新产品"），AI 会自动启动 **deep-research 工作流**——7 步独立调研（大纲生成 → 搜索需求拆分 → 逐页搜索（通过多 AI 浏览器自动化）→ 汇总 → 结构化分析 → 叙事构建 → 视觉策略），产出结构化调研产物后进入主流程生成幻灯片。
