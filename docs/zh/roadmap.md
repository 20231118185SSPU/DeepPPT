...
- **页间转场默认开启，元素级动画改为 opt-in** — 默认导出只保留 `fade` 页间转场；元素级入场动画默认关闭，需通过 `-a auto` 或 `animations.json` 启用，旁白音频 / 视频导出同步上线([`workflows/stages/generate-audio.md`](../../skills/ppt-master/workflows/stages/generate-audio.md))
...
- **视觉自检工作流**（[`workflows/stages/visual-review.md`](../../skills/ppt-master/workflows/stages/visual-review.md)） — 按 rubric 逐页自查 AI 生成的 SVG
