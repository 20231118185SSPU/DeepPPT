...
## Log
### 2026-08-05 — OfficeCLI Phase 4 契约修复：trace duration/隐私、run_summary 消费、smoke Test 16/17、init --project-dir
- **Files**: `skills/ppt-master/scripts/native_revision_pptx.py`（新增 `--project-dir`（对齐 native_enhance 先例，供 fixture/CI 在 .tmp 建项目）；`officecli_probe`/`officecli_validate` trace 事件 + 全部 7 个 officecli 事件补真实 `duration_ms`（§9.3）；修复 `_postflight` PASS trace 引用未定义 `t0` 的 NameError）、`skills/ppt-master/scripts/office_source_inspect.py` / `office_source_repair.py`（`officecli_probe`/`officecli_validate` 事件 + duration）、`skills/ppt-master/scripts/run_summary.py`（fallback stages 消费 8 个 officecli ops）、`skills/ppt-master/scripts/smoke_check.py`（新增 Test 16 native revision fixture 生命周期、Test 17 source inspect + repair 生命周期；提取 `_officecli_binary_installed` 共享守卫）
- **Reason**: `plans/officecli-deep-integration-agent-brief.md` Phase 4 验收复跑发现：§9.3 要求的 `officecli_probe`/`officecli_validate` 事件缺失且事件无 duration；run_summary 不消费 officecli ops（stages 空）；smoke integration 无 native revision / inspect / repair fixture 契约（P4.5 要求 CI 可跑）；native_revision init 无 `--project-dir`（smoke/CI 会污染 projects/）。
- **Before**: trace 只有 5 个事件且无 duration；run_summary stages 不显示 officecli 操作；Test 16/17 不存在；CI 仅能验证 runtime/bridge。
- **After**: 8 个稳定 operation 全部埋点（含 probe/validate）并携带真实 duration；run_summary fallback stages 显示 officecli 各操作耗时（实测 native_revision_inspect 3918ms / office_source_inspect 8049ms / officecli_probe 455ms）；Test 16/17 在 Ubuntu CI 可无头运行（COM 不可用自动 not_available，不伪装通过）；init `--project-dir` 可将 standalone 项目建到 .tmp。
- **Verified**: trace 隐私负测（TOP-SECRET/机密文本/props/raw XML/凭据标记全部不在 trace；事件体量受限）；`/api/state` 三键（officecli_runtime ready/1.0.143 / office_sources 计数 / native_revision applied+svg_divergence:true）且无 apply 写接口（ALLOWED_ACTIONS 仅 start-confirm/start-preview/run-quality）；run_summary rc=0 + stages 消费；smoke integration 291/0/4/295（Test 15/16/17 全 PASS）；full 180/0/4/184、skip-help 89/0/3/92、governance 181/0/4/185；guard rc=0；四顶层 route 文件未增加。
- **Risk**: low（纯可观测性增量 + 测试契约；`--project-dir` 默认行为不变）
- **Human reviewed**: pending

### 2026-08-05 — OfficeCLI Phase 3 契约修复：inspect outline 计数/legacy 标记、repair 校验面与产物、合法 DOCX/XLSX fixtures
- **Files**: `skills/ppt-master/scripts/office_source_inspect.py`（`view outline` 合并：DOCX tables/images/equations/paragraphs、XLSX 逐 sheet formulas/tables/charts/rows 聚合、PPTX 有限 outline；issues 嵌套修复；manifest 新增 `unsupported_enrichment` 标记 legacy/plugin 格式）、`skills/ppt-master/scripts/office_source_repair.py`（新增 `_repair_plan_errors` 共享校验器：target/parent 存在性 + expect fingerprint 实测；apply 增加 Gate 2 重跑校验；issues readback 与 issues delta 记录与警告；result 增加 markdown hash 与 operation digest）、`skills/ppt-master/fixtures/officecli/rebuild_docx_source.py` + `docx_source.docx`（新增：合法 body-level 表结构、含必需 `w:tblPr`）、`skills/ppt-master/fixtures/officecli/rebuild_xlsx_source.py` + `xlsx_source.xlsx`（新增：openpyxl 默认字体重排为 Excel 顺序以满足校验器、core 时间戳中和、确定性 rezip）、`skills/ppt-master/fixtures/officecli/README.md`（OfficeCLI ledger：三 fixture 预期计数）、`skills/ppt-master/scripts/docs/officecli.md`（Phase 2/3 CLI 文档）
- **Reason**: `plans/officecli-deep-integration-agent-brief.md` Phase 3 验收复跑发现：DOCX tables/images 计数在 `view outline` 而非 stats，XLSX 计数在逐 sheet outline（原 `_XLSX_KEYS` 假键名全空）；`view issues` 的 `{"count":0,"issues":[]}` 被整包入列（假 1 issue）；repair check-plan 缺 target/expect 校验、apply 未重跑校验、result 缺 markdown hash/operation digest/issues delta；无 XLSX fixture 且既有 docx_complex 表嵌套在 `<w:p>` 内（OfficeCLI 解析器跳过，无法对账）。
- **Before**: manifest summary 缺 DOCX 表/图计数与 XLSX 关键计数、issues 恒 1；repair check-plan 只查 allowlist/必填字段；apply 失败/结果缺 provenance 字段；无合法 DOCX/XLSX fixture。
- **After**: manifest 计数与 fixture ledger 一致（docx 2 表 1 图 4 段；xlsx 2 sheets/7 formulas/1 table；pptx 4 slides/13 shapes/1 chart/1 picture）；issues 如实 0；repair 全链路（unconfirmed 拒绝 / stale / missing-target / expect-mismatch rc=1；apply 成功产出 timestamped 修复副本 + 新 Markdown + 三项 hash + op digest + issues delta；batch 中途失败无副本发布且原件不变；converter 失败显式 warning 记录）；fixtures 全部 OfficeCLI validate 干净且 rebuild 字节确定。
- **Verified**: P3.1 多 source manifest（schema/相对路径/hash/计数）✓；P3.2 import hook（Office 触发写 manifest、MD-only 不触发）✓；P3.3 对账（ledger 计数全匹配）✓；P3.4 六项正负例 ✓；P3.5 成功 apply + provenance ✓；P3.6a 原子失败无发布 ✓、P3.6b converter warning 记录 ✓；三 fixture rebuild 确定性 SHA 一致（73221c44…/d2102173…/a0522404…）；smoke 180/0/4/184、integration 289/0/4/293；attribution_guard rc=0。
- **Risk**: medium（inspect/repair 校验面与产物增强 + 新 fixtures；docx_complex 等既有 converter 契约 fixture 未改动）
- **Human reviewed**: pending

### 2026-08-05 — OfficeCLI Phase 2 契约修复：check-plan 目标/expect 校验、apply Gate 2、未寻址 parts 保护、rollback 分类、确定性 fixture
- **Files**: `skills/ppt-master/scripts/native_revision_pptx.py`（新增 `_plan_errors` 共享校验器：target/parent 存在性 + `expect` fingerprint 实测；`apply` 增加 Gate 2 重跑 plan 校验；postflight 新增未寻址 parts 语义保护（canonicalize XML + 字节比较二进制 part，剔除 OfficeCLI 固定新增的 `docProps/custom.xml` 伪影）；`_count_validation_issues` 修复 envelope 嵌套层级（`data.errors`/`data.count`）；`_resolve_get_node` 修复嵌套层级；`quality/officecli_validation.json` 增加 source/candidate issue 计数）、`skills/ppt-master/scripts/officecli_bridge.py`（`run_atomic_batch` rollback 探测修复：`data.summary.atomicRolledBack`）、`skills/ppt-master/fixtures/officecli/rebuild_native_revision_source.py`（确定性 rezip：固定 zip 时间戳 + 内嵌 chart workbook core.xml 时间戳中和）、`skills/ppt-master/fixtures/officecli/README.md`（确定性 rebuild 说明）、`skills/ppt-master/workflows/generate-pptx.md` / `template-fill-pptx.md` / `native-enhance-pptx.md`（三消费 route 最小接线：last-mile native edit opt-in 入口与边界）
- **Reason**: `plans/officecli-deep-integration-agent-brief.md` Phase 2 验收复跑发现：check-plan 未校验 target 存在性与 expect fingerprint（§6.3 规则 6 要求）；apply 未重跑 check-plan（§6.4 第 1 步）；postflight 未保护未寻址对象/masters/theme/motion（§6.4 第 6 步）；`run_atomic_batch` 与 `_count_validation_issues` 的 envelope 解析层级错误导致 rollback 误归 `OFFICECLI_COMMAND_FAILED`、chart 6 条已知错误被计为 1；rebuild 因 zip 时间戳非确定；三消费 route 缺 native-revision 接线（§8 矩阵）。
- **Before**: check-plan 只查 schema/hash/allowlist/必填字段，缺 target/expect；apply 只查 confirmed+hash；postflight 只查 slide count + validate delta；rollback 场景 error_code 为 `OFFICECLI_COMMAND_FAILED`；fixture rebuild 字节不确定；三个 route 文件无 native-revision 提及。
- **After**: check-plan 对每个 op 实测 target/parent 存在性与 `expect`（type/text 等键逐一比对）；apply 在 mutation 前重跑 `_plan_errors`（stale/target-missing 直接中止，plan 保持 confirmed 供重新确认）；postflight 增加未寻址 parts 保护（touched slide 及其 rels 排除，XML canonicalize 比较，二进制 part 字节比较，仅允许新增 `docProps/custom.xml`）；batch 失败正确归 `OFFICECLI_BATCH_ROLLED_BACK`；validate baseline-delta 用真实错误计数（6/6 chart axId 已知错误）；rebuild 字节级确定（连续 rebuild SHA-256 一致）；三 route 接线完成。
- **Verified**: P2.3 六项正负例全过（positive rc=0；raw-set/缺 target/expect 不符/stale hash rc=1；draft apply 拒绝）；P2.4 apply 成功（新 export 唯一、source SHA 不变、COM render ok、6/6 baseline delta）；P2.5 注入 batch 中途失败（rc=1、无 export、source SHA 不变、plan→failed、`OFFICECLI_BATCH_ROLLED_BACK`、temp 清理）；P2.6 divergence（origin=generated_export → `svg_divergence: true`）+ preservation 负例（篡改未触碰 slide3 被检出）/正例/排除逻辑；P2.2 watch URL 200 + mark 稳定路径解析 + selected 空选择合法 envelope；rebuild 确定性 YES；完整 smoke 180/0/4/184、integration 289/0/4/293；attribution_guard rc=0；四路由文件未增加。
- **Risk**: medium（核心脚本校验面增强 + 工作流接线；不影响既有 SVG/converter/template-fill/native-enhance/COM 权威）
- **Human reviewed**: pending

### 2026-08-05 — OfficeCLI Phase 1 契约修复：installer --json 子命令位置 + bridge 错误分类 + smoke Test 15
- **Files**: `skills/ppt-master/scripts/install_officecli.py`（`--json` 支持子命令前后两种位置）、`skills/ppt-master/scripts/officecli_bridge.py`（`run_officecli` 错误分类顺序：非零退出优先 `OFFICECLI_COMMAND_FAILED`，`--json` 请求但 rc=0 且 stdout 非 JSON 才判 `OFFICECLI_INVALID_JSON`）、`skills/ppt-master/scripts/smoke_check.py`（新增 Test 15：OfficeCLI pinned runtime + bridge 契约）
- **Reason**: `plans/officecli-deep-integration-agent-brief.md` Phase 1 验收复跑发现：公共命令 `check --json` / `path --json`（§4.3 固定格式）实际 rc=2（argparse 只认子命令前位置），与 CI 步骤及 change-log 记载不符；未知命令的纯文本报错被误归为 `OFFICECLI_INVALID_JSON` 而非 `OFFICECLI_COMMAND_FAILED`；smoke 无 OfficeCLI 错误路径契约覆盖。
- **Before**: `install_officecli.py check --json` rc=2（unrecognized）；`run_officecli(["frobnicate"])` 返回 `OFFICECLI_INVALID_JSON` 而非 `OFFICECLI_COMMAND_FAILED`；integration smoke 无 OfficeCLI 测试（288 checks）。
- **After**: `install --json` / `check --json` / `path --json` 与顶层 `--json` 均 rc=0 JSON 输出（windows-x86_64 ready/1.0.143/d4d4c10f…）；未知命令（含 `--json` 前缀）归 `OFFICECLI_COMMAND_FAILED`；`--json` 请求且 rc=0 但 stdout 非 JSON 才归 `OFFICECLI_INVALID_JSON`（fail-closed）；纯文本 rc=0（如 `--version`）保持容忍；Test 15 覆盖 installer 公共契约、错误路径 rc≠0、checksum/version 负例 fail-closed（变异 lock 副本，不触碰安装）、bridge probe/envelope/validate。
- **Verified**: `check/path/install --json` rc=0 JSON ready；checksum 负例 `OFFICECLI_CHECKSUM_MISMATCH` rc=1、version 负例 `OFFICECLI_VERSION_MISMATCH` rc=1、bogus 子命令 rc=2、未知 flag rc=2；bridge 7 项特征全过（valid-json/non-zero/timeout/invalid-json/plain-text/envelope-false/probe）；7 个既有 fixture PPTX bridge validate 前后 SHA-256 全不变（4 pass，3 个为既有损坏 fixture 的 `corrupt_file` 报告，非回归）；完整 smoke 180/0/4/184、integration smoke 289/0/4/293（+1 Test 15）、`--skip-help` 89/0/3/92；attribution_guard rc=0。
- **Risk**: low（仅 CLI 解析与错误分类修正，不改变成功路径行为；mutated-lock 负例不修改安装树）
- **Human reviewed**: pending
