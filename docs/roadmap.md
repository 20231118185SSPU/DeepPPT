...
- **Source-less generation**: `deep-research` orchestrator supports "topic only, no source files" with 7-step research flow
- **PPTX export step-change**: SVG clipPath → DrawingML picture geometry, marker → native arrows, output consolidated to `exports/`
- **Chart library expands to 71 templates + three icon libraries** (simple-icons / phosphor-duotone / brand-logo)
...
- **Page transitions on by default; per-element animation opt-in** — default export applies a `fade` page transition only; per-element entrance builds stay off unless enabled with `-a auto` or `animations.json`, alongside recorded narration / video export ([`workflows/stages/generate-audio.md`](../skills/ppt-master/workflows/stages/generate-audio.md))
...
- **Visual self-review workflow** ([`workflows/stages/visual-review.md`](../skills/ppt-master/workflows/stages/visual-review.md)) — rubric-based per-page check of AI-generated SVGs
...
- **Material divergence (a free-text field under §c Audience)** — a main-pipeline content-strategy question added as a **free-text box** under the audience field: the user states in their own words how closely to follow the source vs how freely to reshape it (blank = balanced default). Deliberately not a fixed set of options, not recommended from analyzing the source, and not coupled to page count — it is purely the user's stated intent. However freely they ask, facts **stay sourced** — reorganize / reframe / expand / connect what is in the source, never import facts from outside it (that is the `deep-research` job). The Strategist reads the prose when authoring the §IX outline and records it in `design_spec §I`; it is **not** written to `spec_lock` (the Executor never reads it). `mode` and divergence are orthogonal. Beautify / template-fill freeze content and do not surface this field
